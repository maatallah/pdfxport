...
*Document de présentation destiné aux équipes opérationnelles et décisionnelles (Perspective collective)*
...
### ❌ Le Problème (Avant)
Chaque jour, **nous devons** récupérer manuellement des centaines de fiches et étiquettes de production au format PDF depuis la plateforme **SievalHub (Decoloop)**. 
Ce travail est :
...
* **Un clic pour tout capturer :** **Nous sélectionnons** les projets sur la page web, et le système s'occupe du reste.
* **Téléchargement automatique en arrière-plan :** Les fichiers PDF sont récupérés à la chaîne, renommés précisément selon le numéro de commande, et rangés au bon endroit sans aucune intervention manuelle.
...
Actuellement, **notre projet** s'articule autour de quatre éléments clés qui communiquent ensemble :
...
graph TD
    ...
    E -->|Génère des fiches de production| F[6. Fichiers Excel structurés]
...
1. **L'Extension Web Hassad (Le Capteur) :** Intégrée au navigateur Chrome, elle détecte et mémorise les informations des projets affichés à l'écran. Un indicateur visuel (HUD) moderne s'affiche sur la page pour **nous montrer** la progression en temps réel (ex: `56 / 167 récoltés`).
2. **L'Orchestrateur (Moissonneuse-Serveur) :** Le mini-logiciel qui tourne silencieusement en arrière-plan. Il reçoit les demandes de l'extension et télécharge les PDF à la chaîne.
3. **Le Moulin de Traitement (Moissonneuse-Moulin / Parser) :** 
   C'est le **cerveau intelligent** de **notre système**. Loin d'un simple copieur de fichiers, ce programme :
...
4. **Le Fichier de Sortie (Excel) :** Le Moulin génère des fichiers Excel standardisés, nettoyés et parfaitement structurés, prêts à être importés dans **nos machines** d'impression d'étiquettes de production !
...
Plus besoin d'ouvrir Chrome, d'activer une extension ou de faire tourner un serveur local. Tout se passe dans un unique fichier Excel pour **l'utilisateur** :
...
> * **Historique intégré :** **Nous voyons** directement dans Excel quels fichiers ont été téléchargés avec succès, à quelle date, et lesquels ont échoué.
...
La **solution hybride actuelle** est une réussite totale qui libère déjà **nos équipes** des tâches répétitives et sécurise la chaîne d'impression des étiquettes grâce à la puissance d'analyse de **notre Moulin (Parser)**.

L'**alternative Excel** représente **notre prochaine étape** de maturité : transformer un système informatique multi-composants en un simple outil de bureau universel, accessible à tous, extrêmement robuste et simple à piloter.
